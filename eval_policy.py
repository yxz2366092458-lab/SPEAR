                        ...
                        for j in range(n_lights):
                            ...
                            if perturb:
                                new_state_tensor = new_state_tensor + torch.normal(torch.zeros(new_state_tensor.shape),
                                                                                   torch.ones_like(new_state_tensor) * perturb_size)
                            actions["center{}".format(j)] = self.alg.choose_action(new_state_tensor)
                        rl_dict = {f"center{j}": int(actions[f"center{j}"]) for j in range(n_lights)}
                        h_dict = self.heuristic_rules.get_heuristic_actions(obs, last_change, rl_dict, num_lights_override=n_lights)
                        self.heuristic_rules.apply_to_traffic_action(actions, h_dict)
                    else:
                        h_dict = self.heuristic_rules.get_heuristic_actions(obs, last_change, None, num_lights_override=n_lights)
                        self.heuristic_rules.apply_to_traffic_action(actions, h_dict)
                else:
                    for j in range(n_lights):
                        new_state_tensor = new_local_obs[j, :]
    ...
    def eval_marl(self, perturb_size=1e-1, render=False, eval_dict=None, eval_num=0, seed=0, cutoff=0.05):
        ...
        for i in range(1, self.config.main.eval_iters + 1):
            ...
            if i >= 1000:
                ...
                if self.heuristic_rules is not None:
                    rl_dict = {f"center{j}": int(actions[f"center{j}"]) for j in range(n_lights)}
                    h_dict = self.heuristic_rules.get_heuristic_actions(
                        obs, last_change,
                        rl_dict if self.heuristic_rules.rule == "hybrid" else None,
                        num_lights_override=n_lights,
                    )
                    self.heuristic_rules.apply_to_traffic_action(actions, h_dict)
    ...
    for lam in [1e-4, 1e-3, 1e-2, 1e-1]:
        for alpha in [1e-4, 1e-3, 1e-2, 1e-1]:
            ...
            trainer = LightTrainer(alg=alg, N_ROWS=config.env.n_rows, N_COLUMNS=config.env.n_cols, config=config,
                                   ...
                                   heuristic_rule=args.heuristic_rule,
                                   heuristic_min_switch_time=args.heuristic_min_switch_time)
            trainer.load("./results/PBT_adv_grid2x2_seed0_pop8/final")
            trainer.eval(eval_dict=train_2x2, speed=35, perturb=True, perturb_size=1e-1)
            print("\nevaluating completed successfully!")
